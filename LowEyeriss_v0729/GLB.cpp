				...
				if (out_valid) {
					...
					for (int num_fold = 0; num_fold < cur_fold; num_fold++) {
						...
						for (int i = 0; i < PE_width_act; i++) {
							...
							ofmap_buf[i + num_fold * PE_width + cnt_pass * folding * PE_width][ofmap_column].write(tmp + ofmap_tmp[i]);  //!! This formula can't support folding is larger than 2. 
							ofmap_test_buf[i + num_fold * PE_width + cnt_pass * folding * PE_width][ofmap_column][ofmap_ch].write(ofmap_tmp[i]);	// Only for check intermediate ofmap		
...
void GLB::ofmap_check(void) {
	...
	ifstream psum_file("psum5.dat", ios::in);
				...
				for (int j = 0; j < ofmap_height; j++) {
					for (int k = 0; k < ofmap_width; k++) {
						...
						if (ofmap_test_buf[j][k][i].read() != psum_tmp[i]) {
							cout << endl;
							cout << setw(30) << "ofmap[" << j << "][" << k << "] "
								<<"\treturn: " << ofmap_test_buf[j][k][i].read()
