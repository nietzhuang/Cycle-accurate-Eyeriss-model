...
void SCHEDULER::ifmap_scheduling(void) {
	sc_int<8>*	ifmap_tmp = new sc_int<8> [ifmap_height * ifmap_width];
	...
	for (int i = 0; i < ifmap_height; i++) {
		for (int j = 0; j < ifmap_width; j++) {
			ifmap_tmp[i + j * ifmap_height];  // ifmap_tmp[ifmap_height][ifmap_width];
		}
	}
	ifmap_done = false;
	cnt_ifmap_in = 0;
		...
		if (in_vld.read() && !ifmap_done && !start_sche.read()) {
			...
			for (int cnt_ifmap_tmp = 0; cnt_ifmap_tmp < ifmap_width; cnt_ifmap_tmp++) {
				for (int num_height = 0; num_height < ifmap_height; num_height++) {
					...
					ifmap_tmp[num_height + cnt_ifmap_tmp * ifmap_height] = ifmap[num_height].read();
				}
				wait();
			}
			...
			case RS:
				...
				if (ofmap_width <= PE_width && filter_height <= PE_length) {
					for (long long int num_width = 0; num_width < ofmap_width; num_width++) {
						for (long long int num_height = 0; num_height < filter_height; num_height++) {
							...
							ifmap_out_PE[num_height][num_width].write(
								...
								+ (cnt_ifmap_in + cnt_ofmap * stride)*ifmap_height]);
						}
					}
					// Otherwise give zero for gating power.
					for (int num_width = ofmap_width; num_width < PE_width; num_width++) {
				...
				// Lack of PE_width RS condition
					...
					while (cnt_pass < propass) {
						...
						for (int num_fold = 0; num_fold < cur_fold; num_fold++) {
							...
							for (long long int num_width = 0; num_width < PE_width_act; num_width++) {
								for (int num_height = num_fold * filter_height; num_height < (num_fold + 1) * filter_height; num_height++) {
